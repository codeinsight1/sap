try {
	...
	query = 'select SUM(GROSSAMOUNT) AS AMOUNT,'
			+ polygonString
			+ ' AS COND from "_SYS_BIC"."salesmapAPF1d1.webapp.models/REGION_SALES_BP" group by '
			+ polygonString;
	...
	// make sure the polygon is complete i.e. first and last point are same
	query = 'select PARTNERID,COMPANYNAME,LEGALFORM,LATITUDE,LONGITUDE,SUM(GROSSAMOUNT) AS AMOUNT,'
			+ polygonString
			+ ' AS COND from "_SYS_BIC"."salesmapAPF1d2.webapp.models/REGION_SALES_BP" group by PARTNERID,COMPANYNAME,LEGALFORM,LATITUDE,LONGITUDE,'
			+ polygonString + ' order by SUM(GROSSAMOUNT) desc';
	rs = conn.executeQuery(query);
	...
	// make sure the polygon is complete i.e. first and last point are same
	query = 'select YEAR_OF_SALE,SUM(GROSSAMOUNT) AS AMOUNT,'
			+ polygonString
			+ ' AS COND from "_SYS_BIC"."salesmapAPF1d2.models/REGION_SALES_BP" group by YEAR_OF_SALE,'
			+ polygonString + ' order by YEAR_OF_SALE';
	rs = conn.executeQuery(query);
